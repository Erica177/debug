...
var (
	...
	cmdSearchObjects = &cobra.Command{
		...
		Short: "[NEW] search all address by objects type, and output by page" +
	...
	cmdReachAll = &cobra.Command{
		...
		Short: "[NEW] find path from root to an object all address, filter by key[flags] and output by page",
		Args:  cobra.ExactArgs(1),
		Run:   runReachAll,
	}
...
func runSearchObjectAddressByTypeName(cmd *cobra.Command, args []string) {
	...
	allObjects, ok := objMap[objectType]
	if !ok {
...
func runReachAll(cmd *cobra.Command, args []string) {
	...
	if !ok {
		fmt.Printf("can not find stack info for object type %s", objectType)
		return
	}
